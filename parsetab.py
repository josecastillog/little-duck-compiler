...
_lr_productions = [
  ...
  ('vars -> VAR variables COLON type SEMI_COLON mas_var','vars',6,'p_vars','lex.py',109),
  ('vars -> empty','vars',1,'p_vars','lex.py',110),
  ('variables -> list_ids','variables',1,'p_variables','lex.py',113),
  ('mas_var -> variables','mas_var',1,'p_mas_var','lex.py',117),
  ('mas_var -> empty','mas_var',1,'p_mas_var','lex.py',118),
  ('list_ids -> ID mas_ids','list_ids',2,'p_list_ids','lex.py',121),
  ('mas_ids -> COMMA list_ids','mas_ids',2,'p_mas_ids','lex.py',124),
  ('mas_ids -> empty','mas_ids',1,'p_mas_ids','lex.py',125),
  ('type -> INT','type',1,'p_type','lex.py',128),
  ('type -> FLOAT','type',1,'p_type','lex.py',129),
  ('dec_v -> vars','dec_v',1,'p_dec_v','lex.py',133),
  ('dec_f -> funcs mas_f','dec_f',2,'p_dec_f','lex.py',137),
  ('dec_f -> empty','dec_f',1,'p_dec_f','lex.py',138),
  ('mas_f -> funcs mas_f','mas_f',2,'p_mas_f','lex.py',142),
  ('mas_f -> empty','mas_f',1,'p_mas_f','lex.py',143),
  ('body -> LEFT_CURLY mas_statement RIGHT_CURLY','body',3,'p_body','lex.py',148),
  ('mas_statement -> statement mas_statement','mas_statement',2,'p_mas_statement','lex.py',153),
  ('mas_statement -> empty','mas_statement',1,'p_mas_statement','lex.py',154),
  ('statement -> assign','statement',1,'p_statement','lex.py',159),
  ('statement -> condition','statement',1,'p_statement','lex.py',160),
  ('statement -> cycle','statement',1,'p_statement','lex.py',161),
  ('statement -> f_call','statement',1,'p_statement','lex.py',162),
  ('statement -> print','statement',1,'p_statement','lex.py',163),
  ('assign -> ID EQUAL expresion SEMI_COLON','assign',4,'p_assign','lex.py',168),
  ('expresion -> exp mas_expresion','expresion',2,'p_expresion','lex.py',173),
  ('mas_expresion -> MORE_THAN exp','mas_expresion',2,'p_mas_expresion','lex.py',178),
  ('mas_expresion -> LESS_THAN exp','mas_expresion',2,'p_mas_expresion','lex.py',179),
  ('mas_expresion -> NOT_EQUAL exp','mas_expresion',2,'p_mas_expresion','lex.py',180),
  ('mas_expresion -> empty','mas_expresion',1,'p_mas_expresion','lex.py',181),
  ('exp -> termino mas_exp','exp',2,'p_exp','lex.py',186),
  ('mas_exp -> ADD termino','mas_exp',2,'p_mas_exp','lex.py',191),
  ('mas_exp -> SUBTRACT termino','mas_exp',2,'p_mas_exp','lex.py',192),
  ('mas_exp -> empty','mas_exp',1,'p_mas_exp','lex.py',193),
  ('termino -> factor mas_termino','termino',2,'p_termino','lex.py',198),
  ('mas_termino -> MULTIPLY factor','mas_termino',2,'p_mas_termino','lex.py',203),
  ('mas_termino -> DIVIDE factor','mas_termino',2,'p_mas_termino','lex.py',204),
  ('mas_termino -> empty','mas_termino',1,'p_mas_termino','lex.py',205),
  ('factor -> mas_factor','factor',1,'p_factor','lex.py',210),
  ('mas_factor -> OPEN_PAR expresion CLOSE_PAR','mas_factor',3,'p_mas_factor','lex.py',215),
  ('mas_factor -> signo ID','mas_factor',2,'p_mas_factor','lex.py',216),
  ('mas_factor -> signo cte','mas_factor',2,'p_mas_factor','lex.py',217),
  ('mas_factor -> ADD cte','mas_factor',2,'p_mas_factor','lex.py',218),
  ('mas_factor -> SUBTRACT cte','mas_factor',2,'p_mas_factor','lex.py',219),
  ('signo -> ADD','signo',1,'p_signo','lex.py',224),
  ('signo -> SUBTRACT','signo',1,'p_signo','lex.py',225),
  ('signo -> empty','signo',1,'p_signo','lex.py',226),
  ('cte -> CTE_STRING','cte',1,'p_cte','lex.py',231),
  ('cte -> CTE_FLOAT','cte',1,'p_cte','lex.py',232),
  ('cte -> CTE_INT','cte',1,'p_cte','lex.py',233),
  ('print -> PRINT expresion mas_print SEMI_COLON','print',4,'p_print','lex.py',238),
  ('print -> PRINT CTE_STRING mas_print','print',3,'p_print','lex.py',239),
  ('mas_print -> COMMA expresion mas_print','mas_print',3,'p_mas_print','lex.py',244),
  ('mas_print -> CTE_STRING mas_print','mas_print',2,'p_mas_print','lex.py',245),
  ('mas_print -> empty','mas_print',1,'p_mas_print','lex.py',246),
  ('cycle -> DO body WHILE OPEN_PAR expresion CLOSE_PAR SEMI_COLON','cycle',7,'p_cycle','lex.py',251),
  ('condition -> IF OPEN_PAR expresion CLOSE_PAR body mas_condition SEMI_COLON','condition',7,'p_condition','lex.py',256),
  ('mas_condition -> ELSE body','mas_condition',2,'p_mas_condition','lex.py',261),
  ('mas_condition -> empty','mas_condition',1,'p_mas_condition','lex.py',262),
  ('f_call -> ID OPEN_PAR expresion mas_f_call CLOSE_PAR SEMI_COLON','f_call',6,'p_f_call','lex.py',267),
  ('mas_f_call -> COMMA expresion','mas_f_call',2,'p_mas_f_call','lex.py',272),
  ('mas_f_call -> empty','mas_f_call',1,'p_mas_f_call','lex.py',273),
  ('funcs -> VOID ID OPEN_PAR params CLOSE_PAR L_BRACKET vars body R_BRACKET SEMI_COLON','funcs',10,'p_funcs','lex.py',278),
  ('params -> ID COLON type','params',3,'p_params','lex.py',283),
  ('params -> ID COLON type mas_params','params',4,'p_params','lex.py',284),
  ('params -> empty','params',1,'p_params','lex.py',285),
  ('mas_params -> COMMA params','mas_params',2,'p_mas_params','lex.py',290),
  ('empty -> <empty>','empty',0,'p_empty','lex.py',300),
]
